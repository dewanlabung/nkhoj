<?php
/**
 * TLDExtract: Library for extraction of domain parts e.g. TLD. Domain parser that uses Public Suffix List.
 *
 * @link      https://github.com/layershifter/TLDExtract
 *
 * @copyright Copyright (c) 2016, Alexander Fedyashov
 * @license   https://raw.githubusercontent.com/layershifter/TLDExtract/master/LICENSE Apache 2.0 License
 */

use LayerShifter\TLDExtract\Extract;

/**
 * Extract the subdomain, domain and suffix (TLD) components from a URL.
 *
 * @param string $url  The URL to extract
 * @return array       Associative array with 'subdomain', 'domain', and 'suffix'
 */
function tld_extract($url)
{
    // Inicijalizacija Extract objekta (može automatski preuzeti Public Suffix List)
    $extract = new Extract();

    // Pokreni ekstrakciju
    $result = $extract->parse($url);

    // Vrati rezultat kao asocijativno polje
    return [
        'subdomain' => $result->getSubdomain(),
        'domain'    => $result->getRegistrableDomain(),
        'suffix'    => $result->getSuffix()
    ];
}
