...
<div class="max-w-3xl">
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-5">
        ...
        <span class="text-gray-600 dark:text-gray-300">Create Page</span>
    </div>

    <form method="POST" action="/admin/pages" class="space-y-5">
        @csrf
        @if($errors->any())
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="title" required value="{{ old('title') }}"
                        ...
                        x-data x-on:input.debounce.300ms="if(!document.getElementById('slug-field').value) document.getElementById('slug-preview').textContent = '/pages/'+($event.target.value.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''))">
                </div>
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" name="slug" id="slug-field" value="{{ old('slug') }}"
                ...
                <div>
                    ...
                    <textarea id="page-content-editor" name="content" rows="12"
                        ...
                        class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500 resize-y">{{ old('content') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="menu_position" class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                        ...
                        <option value="top_menu" {{ old('menu_position') === 'top_menu' ? 'selected' : '' }}>Top Menu</option>
                        <option value="main_menu" {{ old('menu_position') === 'main_menu' ? 'selected' : '' }}>Main Menu</option>
                        <option value="footer" {{ old('menu_position') === 'footer' ? 'selected' : '' }}>Footer</option>
                    </select>
                </div>
                <div>
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <textarea name="meta_description" rows="2"
                        ...
                        class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none">{{ old('meta_description') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <button type="submit" class="px-6 py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-sm font-semibold rounded-lg transition-colors">Create Page</button>
        </div>
    </form>
</div>
@push('scripts')
