...
@section('content')
<div class="mb-6">
    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Edit Feed</h1>
...
<form method="POST" action="/admin/rss-feeds/{{ $feed->id }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="language" class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                            ...
                            <option value="ne" {{ $feed->language === 'ne' ? 'selected' : '' }}>Nepali</option>
                            <option value="ar" {{ $feed->language === 'ar' ? 'selected' : '' }}>Arabic</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category_id" class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                            ...
                            <option value="{{ $cat->id }}" {{ $feed->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name_en }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="post_count" value="{{ $feed->post_count }}" min="1" max="100"
                    ...
                    @foreach([
                        ['auto_update',       'Auto Update',                  $feed->auto_update],
                        ['show_read_more',    'Show Read More Button',        $feed->show_read_more],
                        ['add_as_draft',      'Add Posts as Draft',           $feed->add_as_draft],
                        ['generate_keywords', 'Generate Keywords from Title', $feed->generate_keywords],
                    ...
                    <div class="pt-2 border-t border-gray-100 dark:border-gray-700">
                        <p class="text-xs text-gray-400">Imported posts: <span class="font-semibold text-gray-600 dark:text-gray-300">{{ $feed->imported_count }}</span></p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ $feed->name }}" required placeholder="Feed Name"
                            class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="url" value="{{ $feed->url }}" required placeholder="https://example.com/feed.xml"
                            class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="images_source"
                            class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                            <option value="original" {{ $feed->images_source === 'original' ? 'selected' : '' }}>Show Images from Original Source</option>
                            <option value="remote" {{ $feed->images_source === 'remote' ? 'selected' : '' }}>Use Remote URL</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="read_more_text" value="{{ $feed->read_more_text }}" placeholder="Read More"
                            class="w-full text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mb-3 flex items-center gap-3">
                            <img src="{{ $feed->default_image }}" class="w-24 h-16 object-cover rounded-lg border border-gray-200 dark:border-gray-600">
                        ...
                        <div class="border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-xl p-8 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-brand-400 transition-colors">
                            ...
                            <p class="text-sm text-gray-400">Click to upload new image (replaces current)</p>
    ...
    <div class="flex justify-end gap-3 mt-6">
        <a href="/admin/rss-feeds" class="px-5 py-2.5 text-sm font-semibold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            Cancel
        </a>
        <button type="submit"
