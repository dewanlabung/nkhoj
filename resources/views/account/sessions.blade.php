    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
        ...
        <div class="flex items-start justify-between p-6 pb-4">
            <div>
                ...
                <p class="text-sm text-gray-400 mt-0.5 max-w-lg">Manage active sessions and review recent sign-in activity on your account.</p>
            </div>
        ...
        @if(session('success'))
        <div class="mx-6 mb-4 px-4 py-3 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded-xl text-sm">{{ session('success') }}</div>
        @endif

        {{-- Active Sessions --}}
        @if($sessions->isNotEmpty())
        <div class="px-6 pb-2">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Active Sessions</p>
        </div>
        <div class="space-y-1 px-4 pb-4">
            @foreach($sessions as $session)
            @php $isCurrent = $session->session_id === $currentSid; @endphp
            <div class="flex items-center gap-3 p-3 rounded-xl {{ $isCurrent ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700' : 'bg-gray-50 dark:bg-gray-700/40' }}">
                {{-- Device icon --}}
                <div class="w-10 h-10 rounded-xl {{ $isCurrent ? 'bg-green-100 dark:bg-green-800/40' : 'bg-white dark:bg-gray-700' }} border border-gray-100 dark:border-gray-600 flex items-center justify-center flex-shrink-0">
        ...
        <div class="border-t border-gray-100 dark:border-gray-700">
            <div class="px-6 pt-4 pb-2 flex items-center justify-between">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Sign-in History</p>
                <div class="flex items-center gap-2">
                    @php $failed = $loginHistories->where('success', false)->count(); @endphp
                    @if($failed > 0)
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-xs font-semibold rounded-full">
                        <span class="w-1.5 h-1.5 rounded-full bg-red-500 inline-block"></span>{{ $failed }} failed
                    </span>
                    @endif
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 text-xs font-semibold rounded-full">
                        <span class="w-1.5 h-1.5 rounded-full bg-green-500 inline-block"></span>{{ $loginHistories->where('success', true)->count() }} successful
                    </span>
                </div>
            </div>
            <div class="px-4 pb-4 space-y-px">
                @foreach($loginHistories as $h)
                <div class="flex items-center gap-3 py-2.5 px-3 rounded-xl {{ !$h->success ? 'bg-red-50/50 dark:bg-red-900/10' : '' }}">
                    <div class="w-8 h-8 rounded-lg {{ $h->success ? 'bg-gray-100 dark:bg-gray-700' : 'bg-red-50 dark:bg-red-900/20' }} flex items-center justify-center flex-shrink-0 text-sm leading-none">
